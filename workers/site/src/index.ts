...
export default { async fetch(request: Request, env: Env): Promise<Response> {
  ...
  let storeCode='',checkoutCode='';
  try{const r=await fetch(STORE_JS,{headers:{'User-Agent':'DosaToppings-Site-Worker'}});if(r.ok)storeCode=await r.text()}catch{}
  try{const r=await fetch(CHECKOUT_JS,{headers:{'User-Agent':'DosaToppings-Site-Worker'}});if(r.ok)checkoutCode=await r.text()}catch{}
  const injected=html.replace('</body>',`${script(env.API_BASE_URL,storeCode,checkoutCode)}</body>`);
  const headers=new Headers(upstream.headers);headers.set('content-type','text/html; charset=UTF-8');headers.set('cache-control','no-store');headers.set('x-dosatoppings-site','live-catalog');
