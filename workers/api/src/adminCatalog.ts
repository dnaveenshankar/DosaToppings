...
export async function adminCatalogRoute(request: Request, env: Env, ctx: AuthContext): Promise<Response | null> {
  ...
  if (request.method === 'GET' && url.pathname === '/v1/admin/catalog/products') {
    ...
    // The products table uses is_published (not is_active). Keep the admin API
    // aligned with the real schema while exposing variant activity separately.
    const rows = await supabaseAdminRest<any[]>(env, 'products?select=id,category_id,name,slug,description,is_published,created_at,updated_at,product_variants(id,name,sku,price_paise,compare_at_price_paise,stock_threshold,pack_size_value,pack_size_unit,is_active)&order=created_at.desc&limit=500');
  ...
  if (request.method === 'PATCH' && /^\/v1\/admin\/catalog\/products\/[^/]+$/.test(url.pathname)) {
    ...
    for (const key of ['name','slug','description','is_published','category_id']) {
      if (key in body) patch[key] = key === 'category_id' && body[key] !== null ? uuid(body[key], 'category_id') : body[key];
    }
    if ('slug' in patch && (typeof patch.slug !== 'string' || !String(patch.slug).trim())) throw new Response('Product slug is required', { status: 400 });
