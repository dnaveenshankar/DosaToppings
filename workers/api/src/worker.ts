...
async function adminRoute(request: Request, env: Env): Promise<Response | null> {
  ...
  if (request.method === 'POST' && url.pathname === '/v1/admin/refunds') {
    ...
    const result = await supabaseRpc(env, 'request_refund_atomic', {
  ...
  if (parts.length === 4 && parts[2] === 'refunds' && parts[3] === 'approve' && request.method === 'POST') {
    const refundId = uuid(url.searchParams.get('refund_id'), 'refund_id');
    return json(await supabaseRpc(env, 'approve_refund_atomic', { p_refund_id: refundId, p_actor: ctx.userId }));
  }

  if (parts.length === 4 && parts[2] === 'refunds' && parts[3] === 'approve' && request.method === 'PUT') {
    const refundId = uuid(url.searchParams.get('refund_id'), 'refund_id');
    return json(await supabaseRpc(env, 'approve_refund_atomic', { p_refund_id: refundId, p_actor: ctx.userId }));
  }
  return null;
...
async function processRefunds(env: Env) {
  ...
  const rows = await supabaseAdminRest<Array<{ id: string; payment_id: string; amount_paise: number }>>(env,
    'refund_requests?select=id,payment_id,amount_paise&status=in.(approved,processing)&order=created_at.asc&limit=10');
  const credentials = btoa(`${env.RAZORPAY_KEY_ID}:${env.RAZORPAY_KEY_SECRET}`);
  for (const row of rows) {
    try {
      const refund = await supabaseRpc<any>(env, 'start_refund_atomic', { p_refund_id: row.id });
      const payments = await supabaseAdminRest<Array<{ provider_payment_id: string | null; order_id: string }>>(env,
        `payments?select=provider_payment_id,order_id&id=eq.${encodeURIComponent(row.payment_id)}&limit=1`);
