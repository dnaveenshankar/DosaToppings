...
function corsOrigin(request: Request): string {
  ...
  return origin === 'https://www.dosatoppings.in' || origin === 'https://admin.dosatoppings.in' || origin === 'https://bill.dosatoppings.in' ? origin : 'https://www.dosatoppings.in';
}
...
async function handleJsonRoute(fn: () => Promise<Response>, origin: string, fallback: string): Promise<Response> {
  try { return withCors(await fn(), origin); }
    ...
    return withCors(new Response(JSON.stringify({ ok: false, error: error instanceof Error ? error.message : fallback }), { status: 400, headers: { 'Content-Type': 'application/json' } }), origin);
  }
}
function contextResponse(ctx: AuthContext): Response {
  return new Response(JSON.stringify({ ok: true, user_id: ctx.userId, email: ctx.email ?? null, role: ctx.role ?? null, is_active: ctx.isActive, permissions: ctx.permissions ?? [] }), { status: 200, headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' } });
}
...
async function adminOperationsRoute(request: Request, env: Env, ctx: AuthContext): Promise<Response | null> {
  ...
  if (request.method === 'GET' && url.pathname === '/v1/admin/dashboard') {
    ...
    return new Response(JSON.stringify({ ok: true, summary: await dashboardSummary(env) }), { headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' } });
  }
  ...
  if (request.method === 'GET' && url.pathname === '/v1/admin/orders') {
    ...
    const rows = await supabaseAdminRest<any[]>(env, `orders?select=id,user_id,status,total_paise,currency,payment_id,shipping_address_snapshot,billing_address_snapshot,created_at,updated_at&order=created_at.desc&limit=${limit}${filter}`);
    return new Response(JSON.stringify({ ok: true, orders: rows }), { headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' } });
  }
  ...
  if (request.method === 'POST' && statusMatch) {
    ...
    if (typeof body.status !== 'string' || !body.status.trim()) throw new Response('status is required', { status: 400 });
    return new Response(JSON.stringify({ ok: true, result: await setOrderStatus(env, ctx, statusMatch[1], body.status.trim(), typeof body.reason === 'string' ? body.reason.slice(0, 500) : undefined) }), { headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' } });
  }

  if (request.method === 'POST' && url.pathname === '/v1/admin/inventory/adjust') {
    requirePermission(ctx, 'inventory.adjust');
    const body = await request.json() as { variant_id?: unknown; quantity?: unknown; reason?: unknown };
    if (typeof body.variant_id !== 'string' || !body.variant_id) throw new Response('variant_id is required', { status: 400 });
    ...
    return new Response(JSON.stringify({ ok: true, result: await adjustInventory(env, ctx, body.variant_id, Number(body.quantity), typeof body.reason === 'string' ? body.reason.slice(0, 500) : '') }), { headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' } });
  }
  return null;
}
...
export default {
  async fetch(request: Request, env: Env, ctx: WorkerExecutionContext): Promise<Response> {
    ...
    if (url.pathname.startsWith('/v1/customer/reviews') || /^\/v1\/products\/[^/]+\/reviews$/.test(url.pathname)) return handleJsonRoute(async () => (await reviewRoute(request, env, undefined)) ?? new Response('Not found', { status: 404 }), origin, 'Review request failed');
