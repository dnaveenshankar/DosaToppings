...
export function buildNewOrderRecipients(superAdminEmail: string, configured: OperationalRecipient[]): string[] {
  const addresses = new Set<string>(); addresses.add(normalizeEmail(superAdminEmail));
  for (const recipient of configured) { if (!recipient.enabled || !recipient.event_types.includes('new_order')) continue; addresses.add(normalizeEmail(recipient.email)); }
  return [...addresses];
}
...
export async function sendTransactionalEmail(env: Env, recipients: string[], subject: string, html: string, idempotencyKey: string): Promise<{ providerMessageId?: string }> {
  ...
  const response = await fetch('https://api.resend.com/emails', { method:'POST', headers:{Authorization:`Bearer ${env.RESEND_API_KEY}`,'Content-Type':'application/json','Idempotency-Key':idempotencyKey}, body:JSON.stringify({from:'DosaToppings <orders@dosatoppings.in>',to:recipients,subject,html}) });
...
export async function dispatchNewOrderEmail(env: Env, orderId: string, eventId: string): Promise<void> {
  ...
  const order=orders[0]; if(!order||order.status!=='paid')return;
  const ids=roles.map((r)=>r.user_id).filter((id):id is string=>typeof id==='string'); if(!ids.length)throw new Error('No Super Admin is configured');
  ...
  const adminEmails=admins.map((a)=>normalizeEmail(a.email)).filter(Boolean); if(!adminEmails.length)throw new Error('No active Super Admin email is configured');
  const recipients=buildNewOrderRecipients(adminEmails[0],configured); const eventKey=`new_order:${eventId}`;
  const existing=await supabaseAdminRest<any[]>(env,`notifications?select=id,status&event_key=eq.${encodeURIComponent(eventKey)}&limit=1`); if(existing.length&&existing[0].status==='sent')return;
  if(!existing.length) await supabaseAdminRest(env,'notifications',{method:'POST',headers:{Prefer:'return=minimal,resolution=ignore-duplicates'},body:JSON.stringify({event_key:eventKey,event_type:'new_order',order_id:orderId,recipients,status:'processing',attempts:1})});
  try {
    ...
    const result=await sendTransactionalEmail(env,recipients,`New order ${order.order_number} — DosaToppings`,newOrderEmailHtml({orderNumber:order.order_number,customerName,totalRupees:(Number(order.total_paise)/100).toFixed(2),paymentStatus:'Paid',adminUrl:`https://admin.dosatoppings.in/orders/${order.id}`}),`dosatoppings:${eventKey}`);
