...
export async function sendTransactionalEmail(
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'DosaToppings <orders@dosatoppings.in>',
      to: recipients,
      subject,
      html,
    }),
...
export function newOrderEmailHtml(order: {
  ...
  return `<!doctype html><html><body style="font-family:Arial,sans-serif;line-height:1.5">
    <h2>New DosaToppings Order</h2>
    <p><strong>Order:</strong> ${safe(order.orderNumber)}</p>
    <p><strong>Customer:</strong> ${safe(order.customerName)}</p>
    <p><strong>Total:</strong> ₹${safe(order.totalRupees)}</p>
    <p><strong>Payment:</strong> ${safe(order.paymentStatus)}</p>
    ${order.adminUrl ? `<p><a href="${safe(order.adminUrl)}">Open order in Admin</a></p>` : ''}
    <hr><small>Developed by Naveen — naveenshankar.in</small>
  </body></html>`;
}
