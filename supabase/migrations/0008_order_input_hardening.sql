-- Harden order input before any stock reservation or financial side effect.
-- Duplicate variants are rejected so one request cannot create ambiguous line semantics.
...
begin
  ...
  if exists (
    select 1
    from (
      select value->>'variant_id' as variant_id, count(*) as line_count
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    if not (v_item ? 'variant_id') or (v_item->>'variant_id') !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then
      raise exception using errcode='22023', message='invalid_variant_id';
    end if;
    begin
  ...
  values (v_order_number, p_actor_id, 'payment_pending', v_subtotal, 0, 0, 0, v_total, p_billing_address, p_shipping_address, nullif(trim(p_referral_code), ''))
  returning id into v_order_id;

  for v_item in select value from jsonb_array_elements(v_items) loop
    insert into public.order_items(order_id, variant_id, product_name, variant_name, sku, quantity, unit_price_paise, discount_paise, line_total_paise)
    values (v_order_id, (v_item->>'variant_id')::uuid, v_item->>'product_name', v_item->>'variant_name', nullif(v_item->>'sku',''), (v_item->>'quantity')::integer, (v_item->>'unit_price_paise')::bigint, 0, (v_item->>'line_total_paise')::bigint);

    insert into public.inventory_movements(variant_id, movement_type, quantity, reference_type, reference_id, notes, performed_by)
    values ((v_item->>'variant_id')::uuid, 'reservation', -(v_item->>'quantity')::integer, 'order', v_order_id, 'Checkout stock reservation', p_actor_id);
  end loop;

  insert into public.order_status_history(order_id, from_status, to_status, changed_by, reason)
  values (v_order_id, 'cart', 'payment_pending', p_actor_id, 'Order created');

  insert into public.payments(order_id, amount_paise, status)
  values (v_order_id, v_total, 'created')
  returning id into v_payment_id;

  update public.api_idempotency_keys
  set response_status=201, resource_type='order', resource_id=v_order_id,
      response_body=jsonb_build_object('ok',true,'order_id',v_order_id,'order_number',v_order_number,'payment_id',v_payment_id,'status','payment_pending','amount_paise',v_total,'currency','INR')
  where actor_id=p_actor_id and idempotency_key=p_idempotency_key;
...
revoke execute on function public.create_order_atomic(uuid,text,text,jsonb,jsonb,jsonb,text,text) from public, anon, authenticated;
