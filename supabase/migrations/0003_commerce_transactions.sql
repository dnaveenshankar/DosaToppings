-- Atomic commerce primitives: idempotency, order creation, inventory reservation/finalization,
-- and payment reconciliation. All money is integer paise. Client totals are never trusted.
...
begin
  ...
  -- Promotion/coupon/referral/tax/shipping policy is deliberately authoritative here.
  -- Until those policy modules are enabled, these components are zero.
  v_total := v_subtotal;
  ...
  for v_item in select value from jsonb_array_elements(v_items) loop
    insert into public.order_items(
      ...
      0,
      (v_item->>'line_total_paise')::bigint
    );
...
begin
  ...
  if p_success then
    ...
    set provider_payment_id=p_provider_payment_id, provider_order_id=coalesce(p_provider_order_id,provider_order_id),
    ...
    update public.payments set provider_payment_id=coalesce(provider_payment_id,p_provider_payment_id), status='failed', method=p_method, updated_at=now() where id=v_payment.id;
    for v_item in select * from public.order_items where order_id=v_order.id loop
      insert into public.inventory_movements(variant_id,movement_type,quantity,reference_type,reference_id,notes)
...
-- These functions are API/service-role primitives. No browser policy is granted.
