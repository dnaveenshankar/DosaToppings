...
begin
  ...
  if v_cart is null then insert into public.carts(customer_id) values(p_customer_id) returning id into v_cart; end if;
  return v_cart;
end; $$;
...
begin
  ...
  on conflict(cart_id,variant_id) do update set quantity=excluded.quantity, updated_at=now() returning id into v_item;
...
create or replace function public.save_customer_address(p_customer_id uuid,p_address_id uuid,p_label text,p_recipient_name text,p_phone text,p_line1 text,p_line2 text,p_locality text,p_city text,p_state text,p_postal_code text,p_country text,p_is_default boolean)
...
begin
  if length(trim(coalesce(p_recipient_name,''))) < 2 or length(trim(coalesce(p_line1,''))) < 3 or length(trim(coalesce(p_city,''))) < 2 or length(trim(coalesce(p_state,''))) < 2 or length(trim(coalesce(p_postal_code,''))) < 3 then raise exception using errcode='22023', message='invalid_address'; end if;
...
declare v_order public.orders%rowtype; v_from_status public.order_status;
begin
  select * into v_order from public.orders where id=p_order_id for update;
  if not found then raise exception using errcode='P0002', message='order_not_found'; end if;
  v_from_status := v_order.status;
  if p_to_status = v_from_status then return v_order; end if;
  update public.orders set status=p_to_status,updated_at=now() where id=p_order_id returning * into v_order;
  insert into public.order_status_history(order_id,from_status,to_status,changed_by,reason) values(p_order_id,v_from_status,p_to_status,p_actor,p_reason);
  return v_order;
end; $$;
