-- 0010 redefined create_order_atomic without the advisory lock introduced in 0004.
-- Restore that concurrency invariant while retaining customer-owned address snapshots.
...
declare
  v_existing public.api_idempotency_keys%rowtype; v_order_id uuid; v_order_number text; v_subtotal bigint:=0; v_total bigint:=0;
  v_variant record; v_item jsonb; v_qty integer; v_available bigint; v_line_total bigint; v_items jsonb:='[]'::jsonb; v_payment_id uuid; v_variant_id uuid;
begin
  ...
  if (select count(*) from jsonb_array_elements(p_items))<>(select count(distinct value->>'variant_id') from jsonb_array_elements(p_items)) then raise exception using errcode='22023',message='duplicate_variant'; end if;
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    begin v_variant_id:=(v_item->>'variant_id')::uuid; v_qty:=(v_item->>'quantity')::integer; exception when others then raise exception using errcode='22023',message='invalid_item'; end;
    ...
    select pv.*,p.name as product_name into v_variant from public.product_variants pv join public.products p on p.id=pv.product_id where pv.id=v_variant_id and pv.is_active=true and p.is_published=true for update of pv;
    ...
    v_line_total:=v_variant.price_paise*v_qty; v_subtotal:=v_subtotal+v_line_total;
  ...
  v_total:=v_subtotal; v_order_number:='DT-'||to_char(now() at time zone 'Asia/Kolkata','YYYYMMDD')||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,10));
  insert into public.orders(order_number,customer_id,status,subtotal_paise,discount_paise,shipping_paise,tax_paise,total_paise,billing_address,shipping_address,referral_code) values(v_order_number,p_actor_id,'payment_pending',v_subtotal,0,0,0,v_total,p_billing_address,p_shipping_address,nullif(trim(p_referral_code),'')) returning id into v_order_id;
