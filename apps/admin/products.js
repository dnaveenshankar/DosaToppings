(() => {
  ...
  const render=()=>{
    ...
    const rows=flat.filter(x=>{
      const text=x.p.name+' '+(x.p.slug||'')+' '+(x.p.short_description||'')+' '+(x.v?.name||'')+' '+(x.v?.sku||'')+' '+(x.v?variantLabel(x.v):'');
  ...
  document.getElementById('pSave').onclick=async()=>{
    try{
      ...
      const body={name,short_description:document.getElementById('pShort').value.trim(),description:document.getElementById('pDesc').value.trim(),image_url:image||null,slug:document.getElementById('pSlug').value.trim()||slugify(name),category_id:document.getElementById('pCategory').value.trim()||null,is_published:document.getElementById('pPublished').checked};
