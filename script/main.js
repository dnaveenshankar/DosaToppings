...
(function () {
  ...
  const PRELOADER_REMOVE_DELAY = 600;
  ...
  function renderProducts(list) {
    ...
    const SAMPLE_IMG = 'assets/sample.png';
    const featuresMap = {
    ...
    list.forEach(prod => {
      ...
      const features = featuresMap[prod.id] || ['Small-batch roasted', 'Stone-ground texture', 'Perfect for dosa & rice'];
      const pointsHTML = `<ul class="flip-points">${features.map(f => `<li>${esc(f)}</li>`).join('')}</ul>`;
      const imgSrc = prod.img || SAMPLE_IMG;

      col.innerHTML = `
        <div class="flip-card" tabindex="0" data-id="${prod.id}" aria-label="${esc(prod.name)} - product card">
          <div class="flip-inner">
            <div class="flip-front">
              <div class="flip-img-wrap" style="background-image:url('${imgSrc}');">
                <img src="${imgSrc}" alt="${esc(prod.name)}" class="cover" />
                <div class="price-pill">₹${prod.price}</div>
              </div>
              <div class="card-caption">
                <div class="d-flex justify-content-between align-items-center">
                  <div>
                    <h5 class="mb-0">${esc(prod.name)}</h5>
                    <small class="text-muted">${esc(prod.category)}</small>
                  </div>
                  <div class="text-end"><div class="price-now visually-hidden">₹${prod.price}</div></div>
                </div>
              </div>
            </div>

            <div class="flip-back" aria-hidden="true">
              <h6 class="mb-1">${esc(prod.name)}</h6>
              <p class="small mb-1">${esc(prod.desc)}</p>
              ${pointsHTML}
              <div class="small text-muted mt-2">Category: ${esc(prod.category)} | Price: ₹${prod.price}</div>
            </div>
          </div>
        </div>
      `;
      ...
      // Flip behaviour: mobile toggles, desktop uses hover via CSS
  ...
  function setupSectionObserver() {
    try {
      ...
      const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          ...
          if (entry.isIntersecting) {
            Object.values(idToNav).forEach(n => n.classList.remove('active'));
      ...
      // graceful fallback
      console.warn('Section observer failed', err);
  ...
  function footerAndGoTop() {
    ...
    if ('IntersectionObserver' in window) {
      const obs = new IntersectionObserver((entries, ob) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            footer.querySelectorAll('.fade-in-up').forEach((el, i) => setTimeout(() => el.classList.add('visible'), i * 120));
            ob.disconnect();
          }
        });
      }, { threshold: 0.08 });
  ...
  function syncNavHeight() {
    ...
    function setNavVar() {
      ...
      document.querySelectorAll('main').forEach(m => { if (m && m.style) m.style.paddingTop = `calc(${h}px + 6px)`; });
      const margin = h + 8;
      document.querySelectorAll('header#home, .hero, main section').forEach(el => { if (el && el.style) el.style.scrollMarginTop = margin + 'px'; });
    }

    window.addEventListener('load', () => setTimeout(setNavVar, 90), { passive: true });
    window.addEventListener('resize', () => { clearTimeout(resizeTimer); resizeTimer = setTimeout(setNavVar, 120); }, { passive: true });
  ...
  function initAfterLoad() {
    try {
      ...
      if (preloader) {
        ...
        setTimeout(() => { if (preloader && preloader.parentNode) preloader.parentNode.removeChild(preloader); }, PRELOADER_REMOVE_DELAY);
      }
    } catch (e) { /* ignore */ }
  ...
  function onWindowLoadEnforcePreloader() {
    ...
    setTimeout(initAfterLoad, remaining);
  }

  if (document.readyState === 'complete') onWindowLoadEnforcePreloader();
  else window.addEventListener('load', function onLoad() { window.removeEventListener('load', onLoad); onWindowLoadEnforcePreloader(); }, { passive: true });
